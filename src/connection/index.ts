...
export type { ConnectionHandle, ConnectionOutcome, ReconnectConfig, ResolvedReconnectPolicy } from './connection.ts'
...
// Connection plumbing a multi-server supervisor drives with: one handle per
// server, the reconnect policy that handle runs under, the defaults that policy
// falls back to, and the `serverName` grammar a dict key must satisfy.
export { RECONNECT_DEFAULTS, resolveReconnectPolicy, SERVER_NAME_PATTERN, startConnection }
