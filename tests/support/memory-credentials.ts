/**
 * In-memory credentials provider fixture, vendored from the dsh credentials
 * package's own test support so this workspace's suites run without reaching
 * into the harness tree. One always-writable `memory` source seeded from
 * plugin config; identical behavior, imports rewritten to the package face.
 */
